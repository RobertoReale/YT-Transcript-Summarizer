...
this file says how to behave while doing it.
...
```
npm test       # exit 0 (all test suites pass)
```

Run before every commit, not once at the end. A phase that ends with one commit
containing six tasks is a phase that cannot be reviewed or reverted.

## Committing

- One task, one commit, Conventional Commit message (e.g. `refactor(config): ...`, `feat(sidepanel): ...`).
...
- you would have to add an external npm/CDN dependency the task did not name.
